...
public class ShopKyGuiService {
    ...
    public List<ItemKyGui> getItemCanKiGui(Player pl) {
        ...
        pl.inventory.itemsBag.stream().filter((it) -> (it.isNotNullItem() && ((it.template.type < 5 && it.template.type >= 0) || it.template.type == 12 || it.template.type == 27 || it.template.type == 33 || it.template.type == 29))).forEachOrdered((it) -> {
            for (ItemOption itc : it.itemOptions) {
                if (itc.optionTemplate.id == 87) {
    ...
    public byte getTabKiGui(Item it) {
        if (it.template.type >= 0 && it.template.type <= 2) {
            ...
            return 1;
        } else if (it.template.type == 29 || it.template.type == 11 || it.template.type == 27) {
            return 2;
        } else  {
            return 3;
    ...
    public void KiGui(Player pl, int id, int money, byte moneyType, int quantity) {
        try {
            if (pl.inventory.gem < 50000) {
                Service.getInstance().sendThongBao(pl, "Bạn cần có ít nhất 50k ngọc xanh để làm phí đăng bán");
                return;
            }
